...
# 从固定本数以下随机选出一个法术
static func get_random_magic_under_level(level: int) -> MagicInfo:
	var allMagicInfo = GameManager.allMagicInfo
	var validInfo: Array[MagicInfo] = []
	for magicInfo in allMagicInfo:
...
# 查询id法术
static func get_magic_by_id(id: int) -> MagicInfo:
	var allMagicInfo = GameManager.allMagicInfo
	for magicInfo in allMagicInfo:
		if magicInfo.id == id:
...
# 查询法术名称
static func get_magic_by_name(name: String) -> MagicInfo:
	var allMagicInfo = GameManager.allMagicInfo
	for magicInfo in allMagicInfo:
